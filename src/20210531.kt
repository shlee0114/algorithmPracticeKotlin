...
class Fir_20210531 {
    ...
    fun solution(priorities: IntArray, location: Int): Int {
        ...
        for(i in priorities.indices){
            stack.push(intArrayOf(priorities[i], i))
        }

        //첫 한 바퀴 이후는 기존에 저장한 스택의 길이만큼 반복하면서 임시 스택에 위에 했던 것과 동일하게 넣으면서 최대 값과 비교한다.
        var tmpStack = CustomStack()
        while (true){
            if(insertValueStack(tmpStack, stack)){
                return answer
            }
            if(stack.isEmpty()){
                stack = tmpStack
    ...
    private fun insertValueStack(tmpStack : CustomStack, stack : CustomStack) : Boolean{
        stack.pop()?.let {
            if((it as IntArray)[0] >= this.maxVal) {
                answer++
                if(it[1] == location){
                    return true
                }
                getMaxVal(tmpStack, stack)
            }else{
                tmpStack.push(it)
            }
        }
    ...
    //stack : 최대 값을 구할 스택
    private fun getMaxVal(tmpStack : CustomStack,stack: CustomStack){
        this.maxVal = 0
        loopStackGetMax(tmpStack)
        loopStackGetMax(stack)
    }
